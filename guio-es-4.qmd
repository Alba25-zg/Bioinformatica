...
```
tendre entonces un nuevo archivo .txt en la carpeta de bioinformatica y ese archivo lo tengo que traer a R
...
## Desafío
1. ¿Qué diferencia hay entre las líneas 7 y 8 de la tabla anterior? y puedo ver solo las lienas 7 y 8 poniendo en la consola, genomas[7:8,], solo se diferencia en la assembly accession en GCF y GCA, es solo por una cuestion puramente tecnica

2. Busca una función de R que te permita identificar las líneas repetidas en un marco de datos. una funcion de R que permite hacer eso sería `duplicated()` la cual devuelve un valor TRUE o FALSE para cada fila.
:::
...
grande para mostrarlo en pantalla completo. ponerlo sin las comillas y el parentesis de fuera
...
o sea he creado solo resumen pero me faltan los otros 4 para completar la tabla puedo ir haciendo la misma orden pero cambiando el nombre de CDS e ir uno a uno y para unir todas las filas, uso resumen_total con rbind, resumen_total <- rbind(resumen, resumen_exon, resumen_gene, resumen_mrna, resumen_region) y con kable(resumen_total) %>% kable_styling() puedo visualizar la tabla
:::
