...
el programa [`exonerate`](https://www.animalgenome.org/bioinfo/resources/manuals/exonerate/index.html), sino en R tambien hay un paquete llamado DECIPHER, ya que instalar EMBOSS puede ser complicado
...
| cp a b    | Crear una copia "b" del archivo "a" | "/home/usuario/Descargas/guio-es.qmd" o tomando el punto de vista de la posicion de descargas (donde esta el documento que nos interesa) cp ../../../../Descargas/ guion.qmd
| mv a b    | Mover el archivo "a" al "b"   |
| mkdir a   | Crear el directorio "a" |yo he creado bioinformatica
...
needle implementa un algoritmo de alineacion de dos secuencias, hay dos amneras de alinearlas local o global, es para alinearlas, lo que empieza con almohadilla son explicaciones y si quiero un alineamiento, no quiero que el script empiece a descargar un cromosoma entero si ya esta de antes, por eso usa ejecution condicional, toda condicion que empieza con if tiene que cerrarse con un fi, la exclamacion sifnifica NO, es decir negar lo que sigue, si utilizao la e es un archivo, si es un directorio es una d. si no existe un archivo que se llame asi, lo descargo con wget, condiconales son desde if a fi, hay varios. Una vez tengo los 2 archivos, los alineo. 
para que el alineamiento salga en pantalla, omito no se que (esto en exonerate): exonerate .... > archivo...
...
```
esas barras invertidas para que funcionen tiene que ir el salto de linea ?
