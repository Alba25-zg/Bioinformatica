...
```{r}
curl_download(
  URLencode('https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(7215) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0'),
  destfile = 'z1.txt')
```
...
```
en el primer ejemplo es una lista de ensamblajes genómicos.
6157: pertenece al taxon
:::
...
todas, se puede saber bajando hasta el final de la tabla en R y le quitas una fila por la cabecera.
2. ¿Qué longitudes tienen? media=1252, summary(adh$base_count)
3. ¿De cuántas especies diferentes son estas secuencias? 308, unique(adh$scientific_name)
4. ¿Cuándo se publicaron las más recientes? head(adh$first_public), necesito ordenarlo y el orden alfabetico coincide con el orden. sort(adh$first_public) las ordena peor pone primero las mas antiguas. tail(sort(adh$first_public))
...
limit  <- 'limit=0'
URL <- paste0(portal, result, query, fields, limit)
curl_download(URLencode(URL), destfile = 'drosophila.txt')
```
:::
el numero de taxonomia de drosophila es 7215
